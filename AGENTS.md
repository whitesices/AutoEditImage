...
- No git repo initialized yet; `.Codex/settings.local.json` is gitignored (contains API key)
...
Hooks are configured in `.Codex/settings.local.json`: a `PostToolUse` hook runs `py_compile` on modified `.py` files after every `Write`/`Edit` to catch syntax errors immediately.
